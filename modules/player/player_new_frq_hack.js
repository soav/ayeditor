...
var chipFrq = 1536000 //1773400;  // 1773400 - Original ZX, 1750000 - Pentagon (3546895/2=1773447.5)
var chipChBVol = 0.75;  //???
var chipFrameFrq = (chipFrq / 1773400 * 70908) //70908; // 70908 - Original ZX, 71680 - Pentagon
...
var emulQty = 4;
var emulErr = chipFrq / 1773400;

var stepsPerBuffTN = ((chipFrq / context.sampleRate) * bufferSize) / 8 * emulQty;
var stepsOutTN = stepsPerBuffTN/bufferSize;

var frameFrqTick = 0;
var frameFrqTickMax = (context.sampleRate / frameFrq) / 2;

var chipEmulOverStep = chipFrq / 8 * emulQty / context.sampleRate;
    ...
    if (chipIsPlay) {
        for (var chip = 0; chip < chipQty; chip++) {
            ...
            while (chipsData[chip].emulCnt > 0) {
                ...
                // frame counter
                chipsData[chip].frameCnt += 16 / emulQty;
                
                
                // envelope counter
                chipsData[chip].envCnt++;
                if (chipsData[chip].envCnt >= chipsData[chip].envPer * emulQty * emulErr) {
                    chipsData[chip].envCnt = 0;
                }
                ...
                // noise
                chipsData[chip].nCnt++;
                if (chipsData[chip].nCnt >= chipsData[chip].noisePer * emulQty * emulErr) {
                    chipsData[chip].nCnt = 0;
                ...
                for (ch = 0; ch < 3; ch++) {
                    ...
                    if (chipsData[chip].tCnt[ch] >= chipsData[chip].tonePer[ch] * emulQty * emulErr) {
                        chipsData[chip].tCnt[ch] = 0;
        ...
        for (var i = 0; i < e.outputBuffer.length; i++) {
            ...
            for (var chip = 0; chip < chipQty; chip++) {
                for (var ch = 0; ch < 3; ch++) {
                    var out = 0;
                    for (var j = 0; j < stepsOutTN; j++) {
                        out += chipsChannelsBuff[chip][ch][idx + j];
                    }
                    output[chip][ch][i] = out / stepsOutTN;
                }
            }
            step += stepsOutTN;
        }
